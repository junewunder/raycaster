...
const map = `ryryryryryryryryryryryryryryryryryryryr
r                                     r
y    bg                               y
y    yo                               y
r                                     r
...
class Map extends Array {
  constructor(map) {
    if (Array.isArray(map)) super(...map);else if (typeof map === 'string') {
      super();
...
class RayCaster {
  ...
  press(evt) {
    ...
    switch (code) {
      ...
      case KEY.UP:
      case KEY.W:
        this.input.up = true;break;

      case KEY.LEFT:
      case KEY.A:
        this.input.left = true;break;
  ...
  release(evt) {
    ...
    switch (code) {
      case KEY.RIGHT:
      case KEY.D:
        this.input.right = false;break;

      case KEY.UP:
      case KEY.W:
        this.input.up = false;break;

      case KEY.LEFT:
      case KEY.A:
        this.input.left = false;break;
  ...
  castRays() {
    ...
    for (let theta = this.player.theta; theta < this.player.theta + this.FOV * Math.PI; theta += 0.005) {
      ...
      for (let i in colors) {
        ...
        this.ctx.fillRect(this.scaleAngle(theta) - lineWidth / 2, this.canvas.height / 2 - color.height, lineWidth, color.height * 2);
      }
    }
  }
