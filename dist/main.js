...
class Color {
  ...
  darken(percent) {
    return new Color(this.r - 255 * percent, this.g - 255 * percent, this.b - 255 * percent, this.a);
  }
...
const map = [[CellTypes.red, CellTypes.red, CellTypes.yellow, CellTypes.red, CellTypes.yellow, CellTypes.red, CellTypes.yellow, CellTypes.red], [CellTypes.yellow, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.red], [CellTypes.red, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.yellow], [CellTypes.yellow, CellTypes.air, CellTypes.air, CellTypes.blue, CellTypes.yellow, CellTypes.air, CellTypes.air, CellTypes.red], [CellTypes.red, CellTypes.air, CellTypes.air, CellTypes.green, CellTypes.orange, CellTypes.air, CellTypes.air, CellTypes.yellow], [CellTypes.yellow, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.red], [CellTypes.red, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.air, CellTypes.yellow], [CellTypes.red, CellTypes.yellow, CellTypes.red, CellTypes.yellow, CellTypes.red, CellTypes.yellow, CellTypes.red, CellTypes.red]];
...
class Ray {
  ...
  nextCell() {
    ...
    let i = 0;
    while (true) {
      // this will loop until it returns a value
      this.pos.x += Math.cos(this.theta) * 0.01;
      this.pos.y += Math.sin(this.theta) * 0.01;

      if (Math.round(this.pos.x) !== Math.round(prevPos.x) || Math.round(this.pos.y) !== Math.round(prevPos.y)) // weird indentation
        {
...
class RayCaster {
  ...
  castRays() {
    ...
    for (let theta = this.player.theta; theta < this.player.theta + this.FOV * Math.PI; theta += 0.005) {
      ...
      do {
        ...
        this.ctx.fillRect(this.scaleAngle(theta) - lineWidth / 2, this.canvas.height / 2 - height, lineWidth, height * 2);
  ...
  drawMiniMap() {
    ...
    this.ctx.fillRect(posX - tileWidth / 2, posY - tileWidth / 2, this.map.length * tileWidth + tileWidth, this.map[0].length * tileWidth + tileWidth);
    ...
    this.ctx.beginPath();
    this.ctx.arc(this.player.pos.x * tileWidth + posX, this.player.pos.y * tileWidth + posY, tileWidth / 2, 0, 2 * Math.PI);
    this.ctx.stroke();
...
let world = new RayCaster(map);(function () {
